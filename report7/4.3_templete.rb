...
# puts npda.read_string('abb')
# puts npda.accepting?
# puts npda.current_configurations.inspect
# puts npda.read_string('a')
# puts npda.accepting?
# puts npda.current_configurations.inspect
...
class LexicalAnalyzer < Struct.new(:string)
  GRAMMAR = [
    ...
    { token: 'd', pattern: /do-nothing/     }, # do keyword
...
puts token_rules.inspect

stop_rule = PDARule.new(2, nil, 3, '$', ['$'])
puts stop_rule.inspect

rulebook = NPDARulebook.new([start_rule, stop_rule] + symbol_rules + token_rules)
puts rulebook.inspect
npda_design = NPDADesign.new(1, '$', [3], rulebook)
puts npda_design.inspect
tokens_string = LexicalAnalyzer.new('while ( x < 5) { x = x * 3 }').analyze.join
puts tokens_string.inspect
puts npda_design.accepts?(tokens_string)
